...
SUBROUTINE read_inp(filename)
  ...
  write(*,*) ' Sorry, it only support pressure couplings.'
  ! INITIALIZE THE SEED
  READ(1,*) nseed 
  ...
  if (ipres == 'YES') THEN
    if (ensemble_pres .eqv. .true.) then
...
SUBROUTINE save_xtc(xtcf,istep)
  ...
  DO I = 1, nptot
    COORD(1,I) = REAL(X(I) - box(1)*FlOOR(X(I)/box(1)))
    COORD(2,I) = REAL(Y(I) - box(2)*FLOOR(Y(I)/box(2)))
    COORD(3,I) = REAL(Z(I) - box(3)*FLOOR(Z(I)/box(3)))
  ENDDO
