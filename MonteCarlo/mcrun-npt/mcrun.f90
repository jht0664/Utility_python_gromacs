...
!  use omp_lib
  ...
  integer(kind=8) :: icon
  double precision :: start, speed, intermediate, before_run, inter_time1, inter_time2
  external rand
...
! save initial configure
  ...
  IF( (iconfig .eq. 'YES') .and. (ensemble_exch .eqv. .false.) ) THEN
...
!     call energy_write
      ...
      write(*,'(a,F10.5,1x,a,F10.3,a)') " >> total speed  = ", speed, " (10^6 steps/hours) ", 1.0D0/speed, "(hours/10^6 steps)"
      ! properties
