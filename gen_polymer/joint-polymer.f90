PROGRAM JOINT_PEO
  ...
  DO
    ...
    IF(B5 == NATOM1) THEN
      ...
      DO J=1, NATOM2
        DD1_NEW(J) = DD1(NATOM1) + DD1_NEW(J)
        DD2_NEW(J) = DD2(NATOM1) + DD2_NEW(J)
        DD3_NEW(J) = DD3(NATOM1) + DD3_NEW(J)
      ENDDO
      ...
      DO
        ...
        IF(CHECK_D == 0) THEN
          ...
          DO J=1, NATOM2
            ...
            ENDIF
            IF(J < 4) THEN
              WRITE(12,905) A5+A5_NEW(J), CC1_NEW(J), CC2_NEW(J), NATOM1-1+J, DD1_NEW(J), DD2_NEW(J), DD3_NEW(J)
              ELSE IF(J > 4) THEN
                WRITE(12,905) A5+A5_NEW(J), CC1_NEW(J), CC2_NEW(J), NATOM1-2+J, DD1_NEW(J), DD2_NEW(J), DD3_NEW(J)
            ENDIF
          ENDDO
          ...
          PRINT*, "Should be reset box size!!"
          STOP
        ENDIF
        ...
        IF(CHECK_D == 1) THEN
          ...
          IF(RAND() >= 0.5D0) THEN
            UNIT1 = DD1(NATOM1-3) - DD1(NATOM1)
            UNIT2 = DD2(NATOM1-3) - DD2(NATOM1)
            UNIT3 = DD3(NATOM1-3) - DD3(NATOM1)
            ! axis unit vector is C2-O of B polymer
            ELSE
              UNIT1 = DD1_NEW(5) - DD1(NATOM1)
              UNIT2 = DD2_NEW(5) - DD2(NATOM1)
              UNIT3 = DD3_NEW(5) - DD3(NATOM1)
          ENDIF
          ...
          DO J=1,NATOM2
            DD1_NEW(J) = DD1_NEW(J) - DD1(NATOM1)
            DD2_NEW(J) = DD2_NEW(J) - DD2(NATOM1)
            DD3_NEW(J) = DD3_NEW(J) - DD3(NATOM1)
            ...
            DD1_NEW(J) = ROT1 + DD1(NATOM1)
            DD2_NEW(J) = ROT2 + DD2(NATOM1)
            DD3_NEW(J) = ROT3 + DD3(NATOM1)
          ENDDO
        ENDIF
      ENDDO
    ENDIF
